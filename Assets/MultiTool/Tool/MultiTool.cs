...
public class MultiTool : EditorWindow
{
    ...
    void GOToolGUI(Transform t)
    {
        ...
        if (GUILayout.Button("Deselect")) { Selection.activeGameObject = null; }
        EditorGUILayout.EndHorizontal();
        ...
        if (EditorGUI.EndChangeCheck()) { rotMover = t.position + (t.forward * 1.5f); }
        EditorGUILayout.EndHorizontal();
    ...
    {
        if (rulerToggle)  { RTool(); }
        if (angleToggle)  { ATool(); }
        if (volumeToggle) { VTool(VMODE); }
